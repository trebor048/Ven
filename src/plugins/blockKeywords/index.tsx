    ...
    containsBlockedKeywords(message: MessageJSON) {
        ...
        for (let embedIndex = 0; embedIndex < message.embeds.length; embedIndex++) {
            ...
            for (let wordIndex = 0; wordIndex < blockedKeywords.length; wordIndex++) {
                ...
                const descriptionHasKeywords = embed.description != null && blockedKeywords[wordIndex].test(embed.description);
                const titleHasKeywords = embed.title != null && blockedKeywords[wordIndex].test(embed.title);
